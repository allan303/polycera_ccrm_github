...
def get_round_tank_size(volumn: float, height_diameter: float = 1.2) -> dict:
    ...
    # area = R**2*math.pi
    # area = (height/height_diameter/2)**2 * math.pi
    # volumn = area * height = (height/height_diameter/2)**2 * math.pi * height
    # height**3 = volumn / math.pi * (height_diameter*2)**2
    height = (volumn / math.pi * (height_diameter*2)**2)**(1/3)
    diameter = height/height_diameter
...
def get_round_tank_volumn(height: float, diameter: float) -> float:
    '''根据D，H获得 圆形槽体V'''
    return height*(diameter/2)**2*math.pi
